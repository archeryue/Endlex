...
A self-hosted, single-user replacement for wandb's two functions that matter for solo LLM training: **metrics tracking + dashboard** and **model checkpoint sync** between cloud GPU instances and a home box.

Designed for the ArcherChat workflow: rent 8×H100 to train, push metrics + final weights to a home server over the public internet, then do all evaluation, inference, and chat experiments locally.

## Why not wandb?

- Free-tier storage limits force you to either pay or be selective in ways that hurt iteration
- Self-hosting is the natural fit when you already have a home box with a domain and webpage hosting
- One server for both metrics and weights collapses the whole observability story to a single place
- Building it is genuinely a one-day project — not worth the recurring cost or the dependency
...
| `POST` | `/api/runs/<name>/metrics` | Append a JSON dict to `metrics.jsonl`. Body: `{"step": ..., "train/loss": ..., ...}` |
...
`Tracker` always writes to a local JSONL first (so a flaky network never costs metrics), then async-POSTs to the configured server. Server URL + token come from env (`ENDLEX_URL`, `ENDLEX_TOKEN`). If `ENDLEX_URL` is unset, runs fully offline (local logging only).
...
3. **Cloud-side resilience.** Always save locally on cloud first (`save_checkpoint` already does this). Upload is best-effort, runs in a background thread, with retry+backoff. Never block training on a network call.
4. **HTTPS.** Reuse the existing let's encrypt cert + nginx reverse-proxy in front of FastAPI. Don't ship checkpoints (or bearer tokens) over plain HTTP.
5. **WSL2 inbound.** If the existing webpage hosting already works, do nothing extra. FastAPI must `bind 0.0.0.0`, not `127.0.0.1`. WSL2 mirrored networking (Windows 11) or Windows host port-forwarding both work.
6. **Disk usage.** Keep an eye on `$ENDLEX_DATA/checkpoints/` — those grow fast at 1–2 GB per d24 save. Either be selective in the trainer (only upload best-val_bpb checkpoints) or add a TTL/retention policy on the server (`keep latest N`, `delete older than N days`).
7. **Concurrency.** Multiple cloud runs writing to one server is fine because each writes to its own `<run_name>` dir. But two writers to the same `metrics.jsonl` would interleave — enforce one-writer-per-run by checking a lock file at `init`.

## Roadmap

### v1 — minimum viable, single file each side
- [ ] `server.py` — FastAPI app with the endpoints above
- [ ] `endlex/tracker.py` — `Tracker` class with the wandb-shaped API
...
- [ ] System metrics (GPU util, VRAM, power) from the trainer
